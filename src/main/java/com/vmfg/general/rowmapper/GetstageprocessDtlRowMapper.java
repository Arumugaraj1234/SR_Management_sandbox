...
public class GetstageprocessDtlRowMapper  implements RowMapper<GetstageprocessDtlEntity> {
	private static final Logger logger = LoggerFactory.getLogger(GetstageprocessDtlRowMapper.class);

	@Override
	public GetstageprocessDtlEntity mapRow(ResultSet row, int rowNum) throws SQLException {
		GetstageprocessDtlEntity tm = new GetstageprocessDtlEntity();
		try {
			...
			tm.setProcessId(row.getString("PC_ID"));
			...
			tm.setMasterDocstsDesc(row.getString("DOCUMENT_STATUS_TYPE_DESCRIPTION"));
		} catch (Exception e) {
