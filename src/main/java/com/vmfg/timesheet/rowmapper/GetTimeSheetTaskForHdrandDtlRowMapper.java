...
public class GetTimeSheetTaskForHdrandDtlRowMapper implements RowMapper<GetTimeSheetTaskForHdrandDtlEntity> {
	private static final Logger logger = LoggerFactory.getLogger(GetTimeSheetTaskForHdrandDtlRowMapper.class);

	@Override
	public GetTimeSheetTaskForHdrandDtlEntity mapRow(ResultSet rs, int rowNum) throws SQLException {
		GetTimeSheetTaskForHdrandDtlEntity result = new GetTimeSheetTaskForHdrandDtlEntity();
		try {
			...
			if (columnExists(rs, "DEFAULT_TASK_DTL")) {
				result.setDefaulTaskDtl((rs.getString("DEFAULT_TASK_DTL")));
			}
			
		} catch (Exception ex) {
			logger.error("GetTimeSheetTaskForHdrandDtlRowMapper error " + ex);
		}
